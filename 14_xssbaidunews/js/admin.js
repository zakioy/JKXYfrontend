    ...
    $("#updateModal .btn-primary").click(function() {
        ...
        if($("#uponfocus").is(':checked')){
        	...
        	uponfocusval = 0;
 				...
 				var updatetoken = $("#updatetoken").val();
        // 获得表单数据
        var upjsonNews = {
								"token":updatetoken,
        				"newsid":updateId,
...
function gettoken() {
	// var tokenval;
	 $.ajax({
		 url: "./server/gettoken.php",
		 type: "get",
		 dataType: "json",
		 success:function(data) {
			$("#insert_token").val(data.token);
			$("#updatetoken").val(data.token);
		 }
	 });
}
